...
function tc_display_top_users ($attr, $content = '') {
	//set defaults
	...
	foreach($count as $user_id => $value) {
		...
		if ($attr['show-avatar'] !='no') {
			echo '<div class="tc-avatar"'.$css1.'">' ;
			if ($attr['profile-link'] == 'no' ) echo get_avatar( $user_id , $attr['avatar-size']); 
			else echo '<a class="tc-profile-link" href="' . esc_url( bbp_get_user_profile_url( $user_id) ) . '">' . get_avatar( $user_id , $attr['avatar-size']) . '</a>';
			echo '</div>' ;
		}
		echo '<div class="tc-wrapper"'.$css2.'>' ;
		echo '<div class="tc-content"'.$css3.'>' ;
		if ($attr['show-name'] !='no') {
			if ($attr['profile-link'] == 'no' ) echo get_the_author_meta ('display_name' , $user_id) ;
			else echo '<a class="tc-profile-link" href="' . esc_url( bbp_get_user_profile_url( $user_id) ) . '">' . get_the_author_meta ('display_name' , $user_id) . '</a>';
		}
		//the content!!
			echo $attr['before'].$value.$attr['after']  ;
			echo "</div></div><br>";
