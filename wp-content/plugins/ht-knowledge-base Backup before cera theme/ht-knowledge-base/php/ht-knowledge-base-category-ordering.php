...
if( !class_exists( 'HT_Knowledge_Base_Custom_Tax_Order' ) ){
	class HT_Knowledge_Base_Custom_Tax_Order {
		...
		function __construct(){
			...
			//
			add_action ('admin_menu', array( $this,  'add_ht_kb_ordering_menu' ) );
		...
		*/
		function add_ht_kb_ordering_menu(){
			...
			$menu_title = __('Article Category Ordering', 'ht-knowledge-base');
		...
		*/
		function display_ht_kb_ordering_page(){
            ...
            <div class="wrap">
                ...
                <h2><?php _e( 'Category Order', 'ht-knowledge-base' ) ?></h2>
                ...
                <div class="instructions">
                	<?php _e('Drag and drop the categories to re-order how they appear in the knowledge base index', 'ht-knowledge-base'); ?>
                </div>
                ...
                <form action="edit.php" method="get" id="ht-kb-ordering-form"> 
	                <div id="order-terms">    
	                    ...
	                    <div class="alignleft actions">
	                        <p class="submit">
	                            <a href="javascript:;" class="save-order button-primary"><?php _e( "Update", 'ht-knowledge-base' ) ?></a>
	                        </p>
	                    </div>	                    
	                </div> 
                </form>
		...
		*/
		function enqueue_ht_kb_category_ordering_scripts_and_styles(){
			...
			if(  $screen->base == 'ht_kb_page_ht_kb_ordering_page' ) {
				wp_enqueue_style( 'ht-kb-admin-style', plugins_url( 'css/ht-kb-admin-style.css', dirname(__FILE__) ) );				
				wp_enqueue_script( 'ht-kb-category-ordering-script', plugins_url( 'js/ht-kb-category-ordering.js', dirname(__FILE__) ), array( 'jquery' , 'jquery-effects-core', 'jquery-ui-draggable', 'jquery-ui-widget', 'jquery-ui-mouse', 'jquery-ui-sortable' ), 1.0, true );				
