...
if( !class_exists( 'HT_Knowledge_Base' ) ){
	...
	class HT_Knowledge_Base {
		...
		private $temp_query;
		public $is_single, $is_ht_kb_category_tax, $is_ht_kb_tag_tax, $is_ht_kb_search, $nothing_found, $taxonomy, $term;
		...
		function __construct(){
			...
			add_filter( 'wp_title', array( $this,'ht_kb_archive_title_filter'), 10, 1 );
		...
		*/
		...
		/**
		* Get the slug for the tag taxonom
		...
		*/
		    ...
		    if($count==''){
				...
				if (isset($_COOKIE['Viewed']) && $_COOKIE['Viewed'] == "true") {
				  // Do nothing
				} else {
				  setcookie("Viewed", "true", time()+10800);	
				  $count++;
		 ...
		 * Allow order byHT_KB_POST_VIEW_COUNT_KEY		 * @param (Array) $query Unfiltered query
		...
		function kb_filter_get_comments( $content, $comment, $args ) {
			...
			_e('Vote this comment:', 'ht-knowledge-base');



			ht_voting_comment( $comment->comment_ID );
		...
		*/
		...
		function ht_kb_theme_compat_reset_post( $args = array() ) {
			...
			// Clear out the post related global
		...
		*/
		...
		function ht_kb_comments_template( $comment_template ) {
		     ...
		     if($post->post_type == 'ht_kb'){ 
				...
				if( $ht_knowledge_base_options['comments-display'] ){
		...
		*/
		...
		function ht_knowledge_base_custom_front_page($query) {
		    ...
		    if(!is_admin() && 
		    	...
		    	isset( $ht_knowledge_base_options ) &&

		    	is_array( $ht_knowledge_base_options ) &&

		    	array_key_exists( 'kb-home', $ht_knowledge_base_options ) &&

		    	$ht_knowledge_base_options['kb-home'] ){
		...
		*/
		...
		function ht_kb_archive_title_filter( $title ) {
		...
		*/
		...
		function ht_kb_posts_per_taxonomy( $query ){
			...
			$user_number_posts = ($ht_knowledge_base_options['tax-cat-article-number']) ? $ht_knowledge_base_options['tax-cat-article-number'] : 5 ;
